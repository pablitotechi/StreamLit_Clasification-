...
\title{Clasificación, Regresión y Series de Tiempo en un Dataset de Cumplimiento Financiero:\\
Evaluación con AUC, K-Fold y Calibración}

\author{
Rodrigo Arturo Vásquez Chavarría \\
Pablo Eduardo Garro Telles
}

\date{Costa Rica, Marzo 2026}
...
\begin{center}
\textbf{2026 - I Minería de Datos} \\
Profesor: Dr. Juan de Dios Murillo
\end{center}
...
Los algoritmos de clasificación aprenden una función de decisión. En este estudio se emplearon siete algoritmos:
...
\item Naive Bayes
\item Support Vector Machine con kernel RBF
\end{itemize}
...
En este trabajo se aplicó SMOTE dentro de cada fold de validación cruzada para evitar \textit{data leakage}.
...
Un AUC = 1.0 indica separabilidad perfecta entre clases, mientras que AUC = 0.5 equivale a una clasificación aleatoria \cite{fawcett2006introduction}.
...
El estimador final es la media y desviación estándar de las métricas obtenidas en cada fold \cite{kohavi1995study}.
...
En este trabajo se incluyen modelos lineales y no lineales.
...
Los modelos lineales con regularización introducen un término de penalización sobre los coeficientes:

\begin{itemize}
\item Ridge minimiza $||w||_2^2$
\item Lasso minimiza $||w||_1$
\item ElasticNet combina ambas penalizaciones
\end{itemize}
...
El modelo ARIMA $(p,d,q)$ combina:

\begin{itemize}
\item Proceso autorregresivo de orden $p$
\item Diferenciación de orden $d$
\item Proceso de media móvil de orden $q$
\end{itemize}

La estacionariedad se verifica mediante el test aumentado de Dickey-Fuller (ADF).

El modelo calibrado determina automáticamente el orden óptimo minimizando el criterio AIC \cite{box2015time}.

En este experimento el ARIMA calibrado encontró el orden $(1,0,1)$.
...
Mantiene tres ecuaciones de actualización con parámetros de suavizado \cite{winters1960forecasting}.
...
La unidad LSTM incorpora tres compuertas:
...
Esto mitiga el problema del gradiente desvaneciente presente en RNNs estándar \cite{hochreiter1997lstm}.

En este trabajo se implementó una arquitectura de dos capas LSTM con:
...
El dataset utilizado es el \textbf{Big Black Money Dataset}, que contiene 9,999 registros de transacciones financieras internacionales vinculadas con potenciales actividades de lavado de dinero.

Cada registro incluye 14 variables.
...
\begin{itemize}
\item Parsing de fechas
\item Normalización del monto
\item Transformación logarítmica
\item Codificación categórica
\item Definición de variable objetivo
\item Escalado MinMax
\end{itemize}

\subsection{División Train/Test}

Para clasificación y regresión se utilizó K-Fold Cross Validation.

Para series de tiempo se utilizó división cronológica:

\begin{itemize}
\item 80\% entrenamiento
\item 20\% prueba
\end{itemize}
